...
async def categorias_page(
    request: Request,
    admin_user: Any = Depends(require_admin_for_template)
):
    """Página principal de gestión de categorías"""
    return templates.TemplateResponse("admin_categorias.html", {"request": request})
        ...
        resultado = [
            {
                ...
                "active": cat.active,  # Usar 'active' para compatibilidad con el HTML
...
async def get_imagenes_disponibles():
    """Obtener lista de imágenes disponibles en el directorio de categorías"""
    try:
        imagenes = []
        if os.path.exists(UPLOAD_DIR):
            for ext in ['*.jpg', '*.jpeg', '*.png', '*.gif', '*.webp']:
                for file in glob.glob(os.path.join(UPLOAD_DIR, ext)):
                    filename = os.path.basename(file)
                    imagenes.append({
                        "nombre": filename,
                        "url": f"/static/img/categorias/{filename}"
                    })
...
async def create_categoria_form(
    ...
    try:
        ...
        if imagen and imagen.filename:
            ...
            final_imagen_url = f"/static/img/categorias/{filename}"
        elif imagen_url:
            # Si no hay archivo pero sí URL de texto
...
async def update_categoria_form(
    ...
    try:
        ...
        # Procesar imagen si se proporcionó archivo
        if imagen and imagen.filename:
            ...
            # Si se proporcionó URL de texto (puede ser vacía para borrar)
            categoria.imagen_url = imagen_url if imagen_url else None
