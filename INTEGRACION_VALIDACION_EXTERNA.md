...
### URL del Endpoint
```
POST http://127.0.0.1:8000/api/v1/validate
```

**Importante:** Reemplaza `127.0.0.1:8000` con la URL de producción de tu servidor.
...
### 1. Validar Usuario en Proyecto
**POST** `/api/v1/validate`

Valida credenciales y acceso de un usuario a un proyecto específico.

### 2. Listar Usuarios de un Proyecto (Para Sincronización)
**GET** `/api/v1/proyecto/{proyecto_nombre}/usuarios`

Lista todos los usuarios asignados a un proyecto, incluyendo el hash de contraseña para sincronización.

**Ejemplo:**
```bash
curl http://127.0.0.1:8000/api/v1/proyecto/Ecomerce/usuarios
```
...
**Nota:** Este endpoint incluye el hash de contraseña (`clave_hash`) para permitir sincronización de credenciales entre sistemas.
...
| `datos_usuario.is_active` | boolean | Estado activo/inactivo del usuario |
| `datos_usuario.fecha_vencimiento` | string | Fecha de vencimiento del acceso al proyecto (ISO 8601) |
    ...
    try:
        ...
        if data["valid"]:
            return {
                ...
                "is_active": data["datos_usuario"]["is_active"]
            }
        else:
            return {
...
if resultado["acceso_permitido"]:
    ...
    print(f"🔒 Usuario activo: {resultado['is_active']}")
else:
...
### Listar Usuarios de un Proyecto (Sincronización)

#### Python (Requests)
```python
import requests

def obtener_usuarios_proyecto_sync(proyecto_nombre):
    """
    Obtiene todos los usuarios de un proyecto con hash de contraseña.
    Útil para sincronizar usuarios entre sistemas.
    
    Returns:
        dict: {"proyecto": str, "usuarios": list, "total": int}
    """
    url = f"http://127.0.0.1:8000/api/v1/proyecto/{proyecto_nombre}/usuarios"
    ...
    try:
        ...
        if response.status_code == 404:
            return {
                "error": True,
                "mensaje": f"Proyecto '{proyecto_nombre}' no encontrado"
            }
        ...
        return {
            ...
            "proyecto_activo": data["proyecto_activo"],
...
# Uso - Obtener usuarios para sincronizar
resultado = obtener_usuarios_proyecto_sync("Ecomerce")

if not resultado.get("error"):
    print(f"📊 Proyecto: {resultado['proyecto']} (Activo: {resultado['proyecto_activo']})")
    print(f"👥 Total usuarios: {resultado['total']}")
    
    for usuario in resultado['usuarios']:
        estado_emoji = "✅" if usuario['activo'] else "❌"
        print(f"{estado_emoji} {usuario['username']} ({usuario['email']})")
        print(f"   Vence: {usuario['fecha_vencimiento']}")
        print(f"   Hash: {usuario['clave_hash'][:20]}...")
else:
...
```javascript
async function obtenerUsuariosProyectoSync(proyectoNombre) {
    const url = `http://127.0.0.1:8000/api/v1/proyecto/${encodeURIComponent(proyectoNombre)}/usuarios`;
    ...
    try {
        ...
        if (response.status === 404) {
            return {
                error: true,
                mensaje: `Proyecto '${proyectoNombre}' no encontrado`
            };
        }
        ...
        return {
            error: false,
            proyecto: data.proyecto,
            proyectoActivo: data.proyecto_activo,
            usuarios: data.usuarios,
            total: data.total
        };
    } catch (error) {
...
// Uso - Sincronizar usuarios
const resultado = await obtenerUsuariosProyectoSync('Ecomerce');

if (!resultado.error) {
    console.log(`📊 Proyecto: ${resultado.proyecto} (Activo: ${resultado.proyectoActivo})`);
    console.log(`👥 Total usuarios: ${resultado.total}`);
    
    resultado.usuarios.forEach(usuario => {
        const emoji = usuario.activo ? '✅' : '❌';
        console.log(`${emoji} ${usuario.username} (${usuario.email})`);
        console.log(`   Vence: ${usuario.fecha_vencimiento}`);
        console.log(`   Hash: ${usuario.clave_hash.substring(0, 20)}...`);
    });
} else {
...
**Caso de Uso - Sincronización de Base de Datos:**
```python
# Sincronizar usuarios desde el servidor central a una base local
import sqlite3

def sincronizar_usuarios_locales(proyecto_nombre):
    """Sincroniza usuarios del servidor central a BD local"""
    
    # 1. Obtener usuarios del servidor
    resultado = obtener_usuarios_proyecto_sync(proyecto_nombre)
    
    if resultado.get("error"):
        print(f"Error: {resultado['mensaje']}")
        return
    
    # 2. Conectar a BD local
    conn = sqlite3.connect('usuarios_local.db')
    cursor = conn.cursor()
    ...
    # 4. Insertar o actualizar usuarios
    for usuario in resultado['usuarios']:
        cursor.execute('''
            INSERT OR REPLACE INTO usuarios 
            (email, username, nombre, activo, fecha_vencimiento, clave_hash, proyecto)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
            usuario['email'],
            usuario['username'],
    ...
    print(f"✅ {resultado['total']} usuarios sincronizados en BD local")

# Ejecutar sincronización
sincronizar_usuarios_locales("Ecomerce")
```
```
...
- **Archivo de configuración completa:** [SISTEMA_PROYECTOS_COMPLETO.md](SISTEMA_PROYECTOS_COMPLETO.md)
